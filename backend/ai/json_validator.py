...
class PlacementRoadmapListSchema(BaseModel):
    roadmaps: List[SingleRoadmapSchema] = Field(..., min_items=1)  # Single placement roadmap
...
def validate_placement_ai_output(json_str: str) -> dict:
    """
    Validates that the JSON string conforms to the placement-specific Pydantic schema structure.
    Ensures that phases, topics, hours, mini projects, and resources exist.
    """
    logger.info("Starting placement JSON validation check")
    ...
    try:
        # Validate against the Pydantic schema
        validated = PlacementRoadmapListSchema(**data)
        dumped = validated.model_dump()
        ...
        logger.info("Placement JSON response schema validated successfully")
        return dumped
    except ValidationError as val_err:
        logger.error(f"Schema validation errors: {val_err.errors()}")
        raise ValueError(f"AI response JSON structure is invalid: {val_err.errors()}")
